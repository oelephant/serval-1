...
int glcd_getTouch(void){
    ...
    uint8_t a,b,c,d,e;
    a = spi_exchange(TOUCH, DUMMY);
    __delay_us(400);
    b = spi_exchange(TOUCH, DUMMY);
    __delay_us(400);
    c = spi_exchange(TOUCH, DUMMY);
    __delay_us(400);
    d = spi_exchange(TOUCH, DUMMY);
    __delay_us(400);
    e = spi_exchange(TOUCH, DUMMY);
    __delay_us(400);
...
void glcd_writeConfig(uint8_t addr_offset, uint16_t val){
    ...
    spi_exchange(DEVICE, 0x0F && val);    // lower byte
